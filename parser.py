 ...
 #  A B C       [C] = [B] - [A]        Like Subleq, but with a subtraction target instead of a conditional address  # /sub /subleq
 #  A *B -C     [[C]] = [[B]] - [A]    Indirect addressing, accomplished through negative addresses
 #  shorthand:
 #      A B         [B] = [B] - [A]    (A B B)
 #      A           [A] = 0            (A A A)

 #  A 0 C       if [A] <= 0, GOTO C (Halt if C <= 0)    # /jmp /jmp? /goto /goto?
 #  0 B C       if [B] <= 0, CALL C (Halt if c <= 0)    # /jsr /jsr? /call /call?
 #  0 B 0       if [B] <= 0, RETURN (Halt if return stack is empty)     # /ret /ret? /return /return?
 #  A B 0       perform I/O on [A] with format [B]      # /io /inout
 #                  1:  print character                 # /print /output /out
 #                  2+: print number
 #                  -1: input character with echo       # /input /in
 #                  -2-:input character without echo
 #  A 0 0       PUSH [A] onto the stack                 # /push
 #  0 0 C       POP the top of stack to C               # /pop
 #  0 0 0       HALT                                    # /halt
 ...
 #      /copy /move     ZERO B C
 #      /negate /neg    A ZERO A
 #      /data A         Add A memory locations, all set to 0
 #      /halt

 #  //import    Imports a module and appends its code at the end.  The file name must end in ".slm"
 #                  Modules cannot import other modules.
 #                  May denote optional handle: "//import filename as handle"
 #                      'handle' will be replace every instance of '$' in the module's code.
 #                      Use this to differentiate between different modules.
 #                      If not given a handle, the '$' will be simply stripped off.
                        ...
                        if len(token) > 2:
                            if token[2] == "as":
                                if len(token) == 4:
                                    ...
                                    print('No handle given to //import', mod_name, "as\n", flush=True)
                                    raise ValueError
                            else:
                                print("Unrecognized command after //import", mod_name, "\nShould be 'as handle'\n", flush=True)
                                raise ValueError

                        mod_raw = self.expand_literals(mod_raw)
                        mod_raw = self.format_chars(mod_raw)
                        mod_raw = mod_raw.replace('$', mod_handle)
            ...
            if instr[0] == '/':
                ...
                if instr == "/subleq" or instr == "/sub":                                           # A B C  where [C] = [B] - [A]
                    ...
                    if count == 1:      # unconditional
                        ...
                        self.tokens[i].insert(1, '0')                                               # A is condition, B is destination
                    else:
                        self.macro_fail(instr, token)
                    ...
                    elif count == 2:                                                                # B is condition, C is destination
                        self.tokens[i].insert(0, '0')
                    else:
                        self.macro_fail(instr, token)
                    ...
                    if count == 0:                  # unconditional
                        ...
                        self.tokens[i].insert(0, '0')                                               # B is the condition
                ...
                elif instr == "/io" or instr == "/inout":                                           # A B 0 where A is target and B is format
                    if count == 2:
                        self.tokens[i].append('0')
                    else:
                        self.macro_fail(instr, token)
                ...
                elif instr =="/negate" or instr == "/neg":                                          # A ZERO A   meta command
                    if count == 1:
                        self.tokens[i].append('ZERO')
                        self.tokens[i].append(self.tokens[i][0])
                    else:
                        self.macro_fail(instr, token)
                    ...
                    number = 0
                    if count == 1:
                        number = int(self.tokens[i][0])
                        if number <= 0:
                            print("Can't add", number, "memory locations")
                            self.macro_fail(instr, token)
                        self.tokens[i][0] = "%"
                        for n_times in range(number):
                            self.tokens[i].append('0')
